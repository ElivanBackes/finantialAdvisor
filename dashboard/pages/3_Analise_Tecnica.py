...
if not asset_id_str:
    ...
    result = AssetService().get_latest_analyses(ObjectId(asset_id_str))[AnalysisType.TECHNICAL]
    if result is None:
        st.info("Ainda não há análise técnica para este ativo (implementada na Etapa 2/3).")
    else:
        st.json(result)
